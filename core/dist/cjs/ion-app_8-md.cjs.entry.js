...
const __chunk_10 = require('./chunk-e2b76f53.js');
    ...
    componentDidLoad() {
        __chunk_7.rIC(() => {
            if (!__chunk_2.config.getBoolean('_testing')) {
                new Promise(function (resolve) { resolve(require('./tap-click-7ab1b684.js')); }).then(module => module.startTapClick(__chunk_2.config));
            }
            if (__chunk_2.config.getBoolean('statusTap', __chunk_2.isPlatform(window, 'hybrid'))) {
                new Promise(function (resolve) { resolve(require('./status-tap-ad28ccfe.js')); }).then(module => module.startStatusTap());
            }
            if (__chunk_2.config.getBoolean('inputShims', needInputShims())) {
                new Promise(function (resolve) { resolve(require('./input-shims-df519f6c.js')); }).then(module => module.startInputShims(__chunk_2.config));
            }
...
class RouterOutlet {
    ...
    async componentDidLoad() {
        this.gesture = (await new Promise(function (resolve) { resolve(require('./swipe-back-f0903c01.js')); })).createSwipeBackGesture(this.el, () => !!this.swipeHandler && this.swipeHandler.canStart(), () => this.swipeHandler && this.swipeHandler.onStart(), step => this.ani && this.ani.progressStep(step), (shouldComplete, step, dur) => {
            if (this.ani) {
