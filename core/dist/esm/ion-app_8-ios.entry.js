    ...
    componentDidLoad() {
        rIC(() => {
            if (!config.getBoolean('_testing')) {
                import('./tap-click-c672ad5e.js').then(module => module.startTapClick(config));
            }
            if (config.getBoolean('statusTap', isPlatform(window, 'hybrid'))) {
                import('./status-tap-322700d6.js').then(module => module.startStatusTap());
            }
...
class RouterOutlet {
    ...
    async componentDidLoad() {
        this.gesture = (await import('./swipe-back-77f49c75.js')).createSwipeBackGesture(this.el, () => !!this.swipeHandler && this.swipeHandler.canStart(), () => this.swipeHandler && this.swipeHandler.onStart(), step => this.ani && this.ani.progressStep(step), (shouldComplete, step, dur) => {
            if (this.ani) {
